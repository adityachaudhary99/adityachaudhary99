...
def post_with_retry(query, variables, tries=8):
    """POST to the GraphQL API, retrying transient failures — 429/5xx (the 502s
    the deep LOC recursion triggers) AND connection drops/timeouts — with
    exponential backoff. Returns the last response so existing status checks
    still apply."""
    import time as _t
    r = None
    for attempt in range(tries):
        try:
            r = requests.post('https://api.github.com/graphql', json={'query': query, 'variables': variables}, headers=HEADERS, timeout=30)
        except requests.exceptions.RequestException:
            if attempt < tries - 1:
                _t.sleep(min(60, 2 ** attempt) + 0.5); continue
            raise
...
def recursive_loc(owner, repo_name, data, cache_comment, addition_total=0, deletion_total=0, my_commits=0, cursor=None):
    ...
    request = post_with_retry(query, variables) # not simple_request(): must save the cache file before raising
    if request.status_code == 200:
